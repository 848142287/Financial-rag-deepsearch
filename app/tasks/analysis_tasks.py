...
async def _analyze_text_chunk_async(qwen_service: QwenService, chunk: DocumentChunk) -> Dict[str, Any]:
    ...
    try:
        # 生成摘要
        summary = await glm_service.generate_summary(chunk.content)

        # 提取关键信息
        key_points = await glm_service.extract_key_points(chunk.content)

        # 情感分析
        sentiment = await glm_service.analyze_sentiment(chunk.content)

        # 金融实体识别
        entities = await glm_service.extract_financial_entities(chunk.content)
...
async def _analyze_image_chunk_async(qwen_service: QwenService, chunk: DocumentChunk) -> Dict[str, Any]:
    ...
    try:
        ...
        # 图片内容描述
        description = await glm_service.analyze_image(image_path)

        # 如果是图表，进行趋势分析
        is_chart = chunk.metadata.get("is_chart", False)
        chart_analysis = None
        if is_chart:
            chart_analysis = await glm_service.analyze_chart(image_path)

        # OCR文本提取
        ocr_text = await glm_service.extract_text_from_image(image_path)
...
async def _analyze_table_chunk_async(qwen_service: QwenService, chunk: DocumentChunk) -> Dict[str, Any]:
    ...
    try:
        ...
        # 表格内容理解
        table_summary = await glm_service.analyze_table(table_data)

        # 数据趋势分析
        trend_analysis = await glm_service.analyze_table_trends(table_data)

        # 关键指标提取
        key_metrics = await glm_service.extract_table_metrics(table_data)
...
async def _analyze_formula_chunk_async(qwen_service: QwenService, chunk: DocumentChunk) -> Dict[str, Any]:
    ...
    try:
        ...
        # 公式解释
        explanation = await glm_service.explain_formula(formula)

        # 变量识别
        variables = await glm_service.extract_formula_variables(formula)

        # 计算步骤分解
        steps = await glm_service.breakdown_formula_steps(formula)
...
async def _analyze_mixed_content_async(qwen_service: QwenService, chunk: DocumentChunk) -> Dict[str, Any]:
    ...
    try:
        # 综合分析
        comprehensive_analysis = await glm_service.analyze_mixed_content(chunk.content, chunk.metadata)
...
async def _batch_analyze_images_async(document_id: str, image_paths: List[str], task) -> Dict[str, Any]:
    ...
    for i, image_path in enumerate(image_paths):
        ...
        try:
            # 分析单张图片
            analysis = await glm_service.analyze_image(image_path)
...
async def _generate_insights_async(document_id: str, task) -> Dict[str, Any]:
    ...
    insights = await glm_service.generate_document_insights(all_analyses)

    # 提取关键趋势
    trends = await glm_service.extract_document_trends(all_analyses)

    # 识别风险和机会
    risk_opportunity = await glm_service.identify_risks_and_opportunities(all_analyses)
...
async def _generate_document_summary_async(qwen_service: QwenService, analysis_results: List[Dict]) -> str:
    ...
    return await glm_service.generate_summary(all_text)
