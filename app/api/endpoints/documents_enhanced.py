...
async def upload_with_deduplication(
    ...
    try:
        ...
        async with aiofiles.tempfile.NamedTemporaryFile(delete=False) as temp_file:
            ...
            try:
                ...
                if not text_content.strip():
                    logger.info(f"文件 {file.filename} 是二进制文件，尝试OCR处理")
                    # 这里可以添加OCR逻辑
                    text_content = f"[二进制文件] {file.filename}, 大小: {len(content)} bytes"

                content = text_content

            except UnicodeDecodeError:
                # 无法解码为文本，使用基本信息
                content = f"[二进制文件] {file.filename}, 大小: {len(content)} bytes"
        ...
        document = Document(
            ...
            file_type=os.path.splitext(file.filename)[1] if file.filename else '.txt',
        ...
        try:
            # 使用统一任务管理器处理文档（整合多模态、嵌入、知识图谱等功能）
            task = process_document_unified.delay(str(document.id), file.filename)
            task_id = task.id
            logger.info(f"启动统一文档处理流水线: ID={document.id}, 任务ID={task_id}, 文件={file.filename}")
        ...
        if is_duplicate:
            # 文档重复，状态设为重复
            task_id = None
            logger.info(f"文档重复，跳过后续处理: ID={document.id}, 相似文档ID={existing_document_id}")
            status_message = "检测到重复文档，已跳过处理流程"
            response_status = "duplicate"
            # 更新文档状态为completed（重复但完成）
            document.status = "completed"
            db.commit()
        else:
            # 文档不重复，启动处理流程
            logger.info(f"文档上传成功，开始处理: ID={document.id}")
            status_message = "文档上传成功，正在处理中"
            response_status = "processing"
            # 文档状态保持为processing，由后台任务更新
